...
// ── Cortardo Agent (from-scratch design agent) ─────────────────────────────

export interface CortardoQuestion {
  id: string;
  title: string;
  question: string;
  whyItMatters?: string;
  /** Up to 3 multiple-choice options; the UI always adds an "Other" input. */
  options: { label: string; description?: string }[];
}

/** A screen produced by a Cortardo Agent run (shown in the Screens sidebar panel). */
export interface CortardoScreen {
  name: string;
  ready?: boolean;
  failed?: boolean;
}

/** Design tokens surfaced in the Design sidebar panel. */
export interface CortardoDesignTokens {
...
/** A generated component or uploaded asset shown in the Assets sidebar panel. */
export interface CortardoAssetFile {
  id: string;
  label: string;
  kind: "component" | "upload";
}

/** A base component (shadcn preset "b0" style) selected by the agent for the build. */
export interface CortardoBaseComponent {
  /** Registry id of the base component, e.g. "button", "card", "dialog". */
  id: string;
  /** Human-readable display name, e.g. "Button". */
  name: string;
  /** One-line justification for picking this component. */
  reason?: string;
}

/**
 * A base component after a parallel edit agent rewrote it to match the
 * product's design tokens + design plan. `source` is the full .tsx file.
 */
...
export const cortardoAgentRuns = pgTable("cortardo_agent_runs", {
  id: uuid("id").primaryKey().defaultRandom(),
  projectId: uuid("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }),
  prompt: text("prompt").notNull(),
  title: text("title"),
  questions: jsonb("questions").$type<CortardoQuestion[]>(),
  answers: jsonb("answers").$type<Record<string, string>>().default({}).notNull(),
  reasoning: text("reasoning"),
  plan: text("plan"),
  reasoningMs: integer("reasoning_ms"),
  screens: jsonb("screens").$type<CortardoScreen[]>(),
  designTokens: jsonb("design_tokens").$type<CortardoDesignTokens>(),
  assets: jsonb("assets").$type<CortardoAssetFile[]>(),
  components: jsonb("components").$type<CortardoBaseComponent[]>(),
  editedComponents: jsonb("edited_components").$type<EditedComponent[]>(),
  status: text("status").notNull().default("thinking"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

export type CortardoAgentRun = typeof cortardoAgentRuns.$inferSelect;
