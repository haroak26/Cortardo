  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        sql`${reviewRuns.stats}->>'headSha' = ${input.headSha}`,
        sql`${reviewRuns.status} in ('queued','running','done')`,
      ),
    )
    .limit(1);
  ...
  try {
    ...
    const [run] = await db
      ...
      .values({
        ...
        stats: { pullRequestNumber: input.pullRequestNumber, headSha: input.headSha ?? null },
      })
    ...
    logger.error("failed to enqueue review", { error: error instanceof Error ? error.message : String(error) });
    return null;
  }
}
...
async function processRun(job: QueuedJob): Promise<void> {
  ...
  try {
    ...
    const request: ReviewRequest = {
      ...
      rules: [],
      learnings: [],
      settings: { autoCommitFixes: Boolean((repository.settings as any)?.autoCommitFixes) },
    };

    const eventStats: StageEvent[] = [];
    const engine = createV3Engine({
      config: { mode: "live", budgets: { globalMs: Number(process.env.CORTADO_GLOBAL_TIMEOUT_MS ?? 480_000) } },
    ...
    await db
      ...
      .set({
        status: "done",
        summary: `${result.summary.issuesConfirmed} confirmed · ${result.summary.issuesFixed} fixed · ${result.summary.issuesVerified} verified`,
    ...
    if (result.findings.length > 0 || result.summary.staticOnly > 0) {
    ...
    logger.info(
      `[${runId.slice(0, 8)}] done: ${result.summary.issuesConfirmed} confirmed, ${result.summary.issuesVerified} verified, ${(result.summary.durationMs / 1000).toFixed(1)}s, $${result.summary.costUsd.toFixed(4)}`,
    );
  } catch (error) {
...
async function persistFindings(runId: string, repository: Repository, result: ReviewResult): Promise<void> {
  const rows = result.candidates.map((candidate) => {
    ...
    return {
      ...
      models: { proof: proof ?? null, review: review ?? null, decision: decision ?? null },
      fix: {
        status: repair?.exit ?? "NOT_ATTEMPTED",
        verified: Boolean(repair?.exit === "VERIFIED" && verification?.passed),
        patch: repair?.finalPatch ?? null,
        reason: repair?.reason ?? null,
        attempts: repair?.attempts.length ?? 0,
      },
      status: repair?.exit === "VERIFIED" && verification?.passed ? "fixed" : "open",
    };
  });
...
async function failRun(runId: string, message: string): Promise<void> {
  await db
    .update(reviewRuns)
    .set({ status: "error", error: message.slice(0, 2000), finishedAt: new Date(), updatedAt: new Date() })
    .where(eq(reviewRuns.id, runId));
}
