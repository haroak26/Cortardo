...
console.log(
  `[cortardo-bot:e2e:hypotheses] repository=${repositoryFullName} (${repositoryId}) pr=#${pullRequestNumber} ` +
    `dryRun=${dryRun} noModel=${process.env.CORTARDO_BOT_NO_MODEL === "1"}`,
);
...
console.log(`[cortardo-bot:e2e:hypotheses] run=${result.runId} head=${result.headSha.slice(0, 8)} published=${result.published}`);
console.log(
  `[cortardo-bot:e2e:hypotheses] stage summary: ${stage.summary} (${(stage.durationMs / 1000).toFixed(1)}s, ` +
...
if (!dryRun) {
  ...
  assert.ok(comments[0].body.includes("## Cortardo Bot review"), "comment is missing the review heading");

  const codegraphComments = await listCortardoBotComments({
    installationId: repository.installationId,
    fullName: repository.fullName,
    pullRequestNumber,
  ...
  console.log(`[cortardo-bot:e2e:hypotheses] verified comment ${comments[0].id} by ${comments[0].author}: ${comments[0].url}`);
  console.log("");
  console.log(comments[0].body.split("\n").slice(0, 30).join("\n"));
  console.log("...");
}

console.log("[cortardo-bot:e2e:hypotheses] PASS");
