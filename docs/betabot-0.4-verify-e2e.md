# Betabot 0.4 — stage 4 (verify) live E2E report
...
```bash
BETABOT_MAX_COST_USD=0.50 BETABOT_API_KEY=... npm run betabot:e2e:verify
```
...
`<!-- betabot:fix... -->`, no stale bot comments, and total cost within budget.
...
the spendable $0.35. `BETABOT_TARGET_COST_USD=0.40` was not reached.
...
1. **The shared gateway key chain picked an exhausted key.** The first run failed with
   ...
   `OPENCODE_MERGE_KEY` was live. The runner needs `BETABOT_API_KEY` set explicitly; this is
   the documented override and the same finding as
   [`betabot-cost-findings.md`](./betabot-cost-findings.md).
...
```bash
npm run betabot:test                                   # 99 unit tests, no network
./node_modules/.bin/tsc --noEmit
BETABOT_MAX_COST_USD=0.50 BETABOT_API_KEY=... \
  npm run betabot:e2e:verify                           # live, publishes to PR 7
BETABOT_DRY_RUN=1 BETABOT_REPOSITORY_ID=... npm run betabot:e2e:verify   # no writes
```
...
review second (`betabot-82a94878`: review `5718743257` at `17:41:57Z`, suggestion review
submitted `17:41:58Z`, 3/3 priority fixes verified, 4 inline suggestions, `$0.1937` of the
budget). An earlier single-comment run (`betabot-b6064a18`, review `5718483151`, 4/4
verified) predates the ordering change.

`2026-09-17` · run `betabot-b6064a18` · PR #7 head `74c5449d` · `BETABOT_PROMPT_CACHE=0`
...
`Betabot verified fix · high · 1 attempt(s)`); every fix verified on the first attempt. Total
...
Both runs used `BETABOT_PROMPT_CACHE=0` after clearing every comment on each PR and
...
| --- | --- | --- | --- | ---: | ---: |
| #6 | `betabot-25ce8bec` | `5719139423` | 2/3 verified (compile) · the third unverified after 4 attempts, no suggestion posted | 3 | $0.4319 |
| #7 | `betabot-79fc7451` | `5719252190` | 3/3 verified (2 compile · 1 source check) | 4 | $0.4230 |
