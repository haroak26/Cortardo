# Betabot gateway cost/cache findings

Probe: `betabot/scripts/probe-gateway.ts` · run 2026-09-17 against `https://api-gateway.merge.dev/v1/ai-sdk`.
Prefix ≈ 3,119 tokens sent twice per model; raw `usage` recorded per call.

## 1. Billing

- **The gateway returns `usage.cost` on every successful call.** Betabot already prefers it
...
- A 402 (`API key spend limit exceeded` / `Credit balance depleted`) is returned when a key is
  ...
  `BETABOT_API_KEY` explicitly for runs, or the resolver tries keys in order.
...
- **Both explicit mechanisms work.** `prompt_cache_key` is a plain body parameter (no message
  format change), so betabot will send `prompt_cache_key: betabot:<role>:<repo>:<pr>:<head>`.
...
- The legacy `openai/gpt-6-astra` (codegen before the sol swap) rejected `max_tokens` and asked
  for `max_completion_tokens`; the existing per-request fallback (`betabot/src/model.ts`) already
