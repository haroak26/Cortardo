...
`npm run bot:learnings -- --dismiss …` manages them.
...
- `npm run bot:test` — 102 unit tests: rules (including the reversed ternary), agent loop
...
- `npm run bot:eval` — fixture recall: PR6 100% (2/2), PR7 100% (6/6); deterministic
  mode catches the rules-only expectations with zero model calls.
- `npm run bot:e2e` — codegraph debug check; calls `runCodegraphStage` directly because
  the pipeline never publishes codegraph.
- `npm run bot:e2e:hypotheses` / `:fixes` — live checks of the single review comment
...
4. **Quality gate**: run `bot:eval` in CI on every prompt/model change.
