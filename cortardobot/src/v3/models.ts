...
export class HttpModelClient implements ModelClient {
  ...
  private maxTokens(task: ModelTask): number {
    ...
    if (this.role === "codegen") return this.config.maxTokensCodegen;
...
export async function preflightModels(
  ...
  const ids = [config.luna, config.terra, config.codegen, config.astra];
  if (!config.apiKey) {
...
export class ModelRouter {
  ...
  constructor(options: {
    ...
    this.clients = {
      ...
      codegen: options.clients.codegen ?? fallback("codegen"),
