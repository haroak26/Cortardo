...
export class E2BSandboxInstance implements Sandbox {
  ...
  async prepare(options: { cloneUrl: string; token: string; ref: string; headBranch?: string }): Promise<void> {
    ...
    if (!checkedOut) throw new Error(`could not check out ${options.ref}: ${refs.join(", ")}`);
  ...
  async install(): Promise<void> {
    if (await this.exists(`${this.root}/node_modules`)) return;
    ...
    if (result.exitCode !== 0) {
      ...
      if (retry.exitCode !== 0) throw new Error(`dependency install failed: ${retry.stderr.slice(-600)}`);
    }
  }
  ...
  async profile(): Promise<RepoProfile> {
    ...
    const hasPnpm = await this.exists(`${this.root}/pnpm-lock.yaml`);
    const hasYarn = await this.exists(`${this.root}/yarn.lock`);
    const packageManager: RepoProfile["packageManager"] = hasPnpm ? "pnpm" : hasYarn ? "yarn" : await this.exists(`${this.root}/package.json`) ? "npm" : "unknown";
    ...
    try {
      testFiles = (await this.list(this.root)).filter((file) => isTestPath(file)).slice(0, 400);
    } catch {
      testFiles = [];
    }
    return {
      ...
      testCommand: testScript ? "npm test --silent" : undefined,
      typecheckCommand,
      buildCommand,
      devCommand,
      scripts,
  ...
  async list(dir = this.root): Promise<string[]> {
    const result = await this.exec(`cd ${sh(dir)} && find . -type f -not -path "./node_modules/*" -not -path "./.git/*" | sed 's|^\\./||' | head -600`, {
      timeoutMs: 60_000,
    });
  ...
  private async resolveDevCommand(port: number): Promise<string> {
    const hasVite =
      (await this.exists(`${this.root}/vite.config.ts`)) ||
      (await this.exists(`${this.root}/vite.config.js`)) ||
      (await this.exists(`${this.root}/vite.config.mjs`));
