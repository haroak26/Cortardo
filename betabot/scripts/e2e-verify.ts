...
const SUGGESTION_MARKER = "<!-- betabot:fix";
...
async function cleanBotConversation(installationId: string | number): Promise<number> {
  ...
  for (const comment of comments) {
    ...
    deleted += 1;
    console.log(`[betabot:e2e:verify] removed old comment #${comment.id} by ${comment.user?.login}`);
  }
  ...
  for (const comment of reviews) {
    ...
    deleted += 1;
    console.log(`[betabot:e2e:verify] removed old review comment #${comment.id} by ${comment.user?.login}`);
  }
  return deleted;
}
...
console.log(
  `[betabot:e2e:verify] repository=${repositoryFullName} (${repositoryId}) pr=#${pullRequestNumber} ` +
    `dryRun=${dryRun} verifyAttempts=${process.env.BETABOT_VERIFY_ATTEMPTS ?? "4"} ` +
    `maxCost=$${process.env.BETABOT_MAX_COST_USD ?? "0.50"}`,
);

if (!dryRun) {
  const removed = await cleanBotConversation(repository.installationId);
  console.log(`[betabot:e2e:verify] conversation is clean (${removed} old bot artifact(s) removed)`);
}
...
console.log(
  `[betabot:e2e:verify] run=${result.runId} head=${result.headSha.slice(0, 8)} published=${result.published}`,
);
console.log(
  `[betabot:e2e:verify] fixes: ${fixReport.totals.generated} generated · ${fixReport.totals.filtered ?? 0} filtered by severity · ` +
    `$${fixReport.usage.totalCostUsd.toFixed(4)}`,
);
console.log(
  `[betabot:e2e:verify] verify: ${verifyReport.totals.verified}/${verifyReport.totals.eligible} verified · ` +
...
for (const fix of verifyReport.fixes) {
  console.log(
    `[betabot:e2e:verify]   ${fix.status.toUpperCase()} ${fix.hypothesisId} (${fix.evidence}) attempts=${fix.attemptsUsed}` +
...
assert.ok(result.body.includes("## Betabot review"), "the published body is not the single review comment");
...
if (!dryRun) {
  ...
  const review = await listBetabotComments({
  ...
  for (const comment of reviewComments) {
    assert.ok(comment.body.includes(SUGGESTION_MARKER), `review comment #${comment.id} is not a betabot suggestion`);
    assert.ok(comment.body.includes("Betabot verified fix"), `suggestion #${comment.id} is not marked verified`);
  }
  assert.equal(
  ...
  console.log(
    `[betabot:e2e:verify] conversation verified: review=${review[0].id} published before ${reviewComments.length} suggestion(s)`,
  );
  console.log("");
...
console.log("[betabot:e2e:verify] PASS");
