 ...
 * harness for betabot, not for the code under review — it proves recall and
 * measures noise before anything is wired into the product.
 *
 * Usage:
 *   npm run betabot:eval
 *   npm run betabot:eval -- --pr 7
 *   npm run betabot:eval -- --deterministic
 */
import pg from "pg";
...
async function resolveRepositoryId(fullName: string): Promise<string> {
  if (process.env.BETABOT_REPOSITORY_ID && (!onlyPr || onlyPr === FIXTURES[0].pullRequestNumber)) {
    return process.env.BETABOT_REPOSITORY_ID;
  }
...
const modelConfig = resolveBetabotModelConfig();
...
for (const fixture of FIXTURES) {
  ...
  const report = await buildHypothesisReport({
    ...
    analyses,
    index,
    modelClient: useModel ? createBetabotModelClient(modelConfig) : null,
    swarmClient: useModel ? createBetabotModelClient(swarmModelConfig(modelConfig)) : null,
    modelConfig,
    swarmConfig,
    installationId: context.installationId,
    fullName: context.fullName,
...
if (failed) {
  console.error("\n[betabot:eval] FAIL: expected findings were missed");
  process.exit(1);
}
console.log("\n[betabot:eval] PASS");
