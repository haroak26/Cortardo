...
const runId = `betabot-e2e-codegraph-${Date.now().toString(36)}`;
console.log(`[betabot:e2e] repository=${repositoryFullName} (${repositoryId}) pr=#${pullRequestNumber} dryRun=${dryRun}`);
...
console.log(`[betabot:e2e] run=${runId} head=${stage.headSha.slice(0, 8)} published=${stage.commentUrl !== null}`);
console.log(`[betabot:e2e] stage summary: ${stage.summary} (${(stage.durationMs / 1000).toFixed(1)}s, ${stage.body.length} chars)`);
...
if (!dryRun) {
  ...
  assert.ok(comments[0].body.includes("## Betabot · Stage 1: codegraph"), "comment is missing the stage heading");
  assert.ok(comments[0].body.includes(stage.summary), "comment is missing the stage summary");

  console.log(`[betabot:e2e] verified comment ${comments[0].id} by ${comments[0].author}: ${comments[0].url}`);
  console.log("");
  console.log(comments[0].body.split("\n").slice(0, 24).join("\n"));
  console.log("...");
}

console.log("[betabot:e2e] PASS");
