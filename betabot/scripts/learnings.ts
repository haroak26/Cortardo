 ...
 *   npm run betabot:learnings -- --repository owner/repo --remove s_ab12cd34ef
 */
import pg from "pg";
...
if (dismiss) {
  ...
  console.log(`[betabot:learnings] dismissed ${dismiss} (learning ${row.id}): ${reason}`);
} else if (remove) {
  ...
  if (matches.length === 0) {
    console.error(`[betabot:learnings] no active learning for ${remove}`);
    process.exit(1);
  }
  for (const row of matches) await storage.deactivateRepositoryLearning(row.id);
  console.log(`[betabot:learnings] removed ${matches.length} learning(s) for ${remove}`);
} else if (list) {
  const rows = await storage.listRepositoryLearnings(repository.workspaceId, repository.id);
  if (rows.length === 0) {
    console.log(`[betabot:learnings] no active learnings for ${repositoryFullName}`);
  } else {
