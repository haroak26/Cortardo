...
import {
  HttpBetabotModelClient,
  codegenModelConfig,
  resolveBetabotModelConfig,
  type BetabotModelConfig,
} from "../src/model.ts";

const CONFIG: BetabotModelConfig = {
...
test("concurrent capability rejections each recover on their own request", async () => {
  ...
  const client = new HttpBetabotModelClient(CONFIG, fetchImpl);
...
test("codegen model config uses minimal reasoning and the sol default", () => {
  ...
  const overridden = resolveBetabotModelConfig({ BETABOT_REASONING_CODEGEN: "high" });
...
test("cached prompt tokens are billed at the cached rate", async () => {
  ...
  const client = new HttpBetabotModelClient({ ...CONFIG, model: "openai/gpt-5.6-luna" }, fetchImpl);
  const completion = await client.complete({ system: "s", user: "u" });
...
test("unknown models are priced conservatively instead of free", async () => {
  ...
  const client = new HttpBetabotModelClient({ ...CONFIG, model: "openai/custom-mystery" }, fetchImpl);
  const completion = await client.complete({ system: "s", user: "u" });
...
test("prompt_cache_key is sent and disabled per request when the gateway rejects it", async () => {
  ...
  const client = new HttpBetabotModelClient(CONFIG, fetchImpl);
  const completion = await client.complete({ system: "s", user: "u", cacheKey: "betabot:swarm:acme/app:7:abcdef" });
  assert.equal(completion.text, '{"ok":true}');
  assert.equal(bodies.length, 2, "the rejected request retried without the cache key");
  assert.equal(bodies[0].prompt_cache_key, "betabot:swarm:acme/app:7:abcdef");
