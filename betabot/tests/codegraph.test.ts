...
test("renders the stage comment with marker, table and file sections", () => {
  ...
  const body = buildCodegraphComment({ report, runId: "betabot-test", version: "0.1.0" });
  assert.ok(body.includes(CODEGRAPH_MARKER));
  assert.ok(body.includes("## Betabot · Stage 1: codegraph"));
  ...
  assert.ok(body.includes("_Run `betabot-test`"));
});
...
test("reports an empty graph without throwing", () => {
  ...
  const body = buildCodegraphComment({ report, runId: "betabot-empty", version: "0.1.0" });
