...
function codegenInput(client: BetabotModelClient, readFile?: (path: string) => Promise<string | undefined>) {
  return {
    repository: "acme/app",
    pullRequestNumber: 7,
...
test("buildFixReport plans and generates a validated draft", async () => {
  ...
  assert.deepEqual(
    ...
    ["betabot:coordinator:acme/app:7:74c5449d"],
    "the terra planner sends one prompt cache key",
  );
  assert.deepEqual(
    [...new Set(codegen.calls.map((call) => call.cacheKey))],
    ["betabot:codegen:acme/app:7:74c5449d"],
    "every codegen turn shares one prompt cache key",
  );

  const body = buildFixesComment({ report, runId: "betabot-test", version: "0.2.0" });
  assert.ok(body.includes(FIXES_MARKER));
  assert.ok(body.includes("## Betabot · Stage 3: fixes"));
...
test("collectSuggestions only plans in-diff edits and keeps out-of-diff ones out", () => {
  ...
  assert.ok(suggestions[0].body.includes("<!-- betabot:fix s_a -->"));
