...
import {
  ...
  swarmModelConfig,
  type BetabotCompleteInput,
  type BetabotModelClient,
  type BetabotModelCompletion,
} from "../src/model.ts";
...
class ScriptedClient implements BetabotModelClient {
...
test("resolves the coordinator and swarm models with env overrides", () => {
  const defaults = resolveBetabotModelConfig({});
  ...
  const overridden = resolveBetabotModelConfig({
    BETABOT_MODEL: "openai/custom-coordinator",
    BETABOT_SWARM_MODEL: "openai/custom-swarm",
    CORTADO_AI_API_KEY: "secret",
    BETABOT_REASONING: "high",
  });
  ...
  const explicit = resolveBetabotModelConfig({ BETABOT_API_KEY: "override", CORTADO_AI_API_KEY: "shared" });
  assert.equal(explicit.apiKey, "override", "BETABOT_API_KEY wins so an exhausted key can be swapped");
});

test("resolves swarm and budget settings with env overrides", () => {
  const defaults = resolveBetabotSwarmConfig({});
  ...
  const overridden = resolveBetabotSwarmConfig({
    BETABOT_SWARM_AGENTS: "4",
    BETABOT_SWARM_CONCURRENCY: "2",
    BETABOT_MAX_COST_USD: "0.15",
    BETABOT_SWARM_TURNS: "5",
    BETABOT_SWARM_TOOLS: "2",
    BETABOT_SWARM_SEARCH: "0",
    BETABOT_LEARNINGS: "0",
  });
