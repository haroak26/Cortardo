...
import { BetabotUsageTracker } from "../src/model.ts";
...
function loopInput(
  ...
  return {
    ...
    deps,
    tracker: new BetabotUsageTracker(SWARM_CONFIG.maxCostUsd),
    config: SWARM_CONFIG,
...
test("runVerifyLoop skips when the cost budget is already spent", async () => {
  ...
  sandbox.setHead(`${REPO_DIR}/src/db.ts`, "const timeoutMs = 30_000;\n");
  const tracker = new BetabotUsageTracker(0.001, { reserveUsd: 0.001 });
  const results = await runVerifyLoop(
...
test("plannedSuggestions prefers the verified set, then drafts, then clears", () => {
  ...
  assert.equal(verified?.label, "Betabot verified fix");
...
test("buildVerifyComment renders statuses, attempts, commands and the marker", () => {
  ...
  const body = buildVerifyComment({ report, runId: "betabot-test", version: "0.4.0" });
  assert.ok(body.startsWith(VERIFY_MARKER));
  assert.ok(body.includes("## Betabot · Stage 4: verify (autmpus loop)"));
...
test("usage tracker unlocks the stage-4 reserve only for the reserved stage", () => {
  const tracker = new BetabotUsageTracker(0.5, { reserveUsd: 0.15 });
