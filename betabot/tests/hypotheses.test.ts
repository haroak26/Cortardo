...
import { createBetabotUsageTracker } from "../src/model.ts";
import type {
  BetabotCompleteInput,
  BetabotModelClient,
  BetabotModelCompletion,
  BetabotModelConfig,
  BetabotSwarmConfig,
} from "../src/model.ts";
import type { CodegraphChangedFile, RepoGraphIndex, HypothesisDismissal } from "../src/types.ts";

const MODEL_CONFIG: BetabotModelConfig = {
...
const SWARM_CONFIG: BetabotSwarmConfig = {
  maxAgents: 6,
  concurrency: 2,
  maxTurns: 3,
  maxToolsPerTurn: 2,
...
class ScriptedClient implements BetabotModelClient {
  ...
  constructor(
    id: string,
    private readonly handler: (call: number, input: BetabotCompleteInput) => string | Error,
  ) {
    this.id = id;
  }
...
test("full swarm run uses both tiers and publishes per-role usage", async () => {
  ...
  assert.deepEqual(
    ...
    ["betabot:coordinator:acme/app:6:a9a43b844bc4"],
    "every terra call shares one prompt cache key",
  );
  assert.deepEqual(
    [...new Set(luna.calls.map((call) => call.cacheKey))],
    ["betabot:swarm:acme/app:6:a9a43b844bc4"],
    "every luna call shares one prompt cache key",
  );

  const body = buildHypothesesComment({ report, runId: "betabot-test", version: "0.2.0" });
  ...
  assert.ok(body.includes("dismiss with `npm run betabot:learnings"));
});
...
test("the cost budget blocks new model calls and says so", async () => {
  ...
  const tracker = createBetabotUsageTracker(swarmConfig);
...
test("deterministic-only mode publishes leads without model usage", async () => {
  ...
  assert.match(report.usage.reason ?? "", /BETABOT_NO_MODEL/);
  assert.equal(report.totals.hypotheses, 1);
  const body = buildHypothesesComment({ report, runId: "betabot-test", version: "0.2.0" });
...
test("renders an empty hypotheses comment without throwing", async () => {
  ...
  const body = buildHypothesesComment({ report, runId: "betabot-empty", version: "0.2.0" });
