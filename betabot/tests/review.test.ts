...
import { buildReviewComment, BETABOT_MARKER, REVIEW_MARKER } from "../src/markdown.ts";
import type {
  FixReport,
  GeneratedFix,
  Hypothesis,
...
test("buildReviewComment is one review with findings, fixes and a collapsed log", () => {
  ...
  assert.ok(body.includes(BETABOT_MARKER));
  assert.ok(body.includes("## Betabot review"));
