...
export const BETABOT_MARKER = "<!-- betabot -->";
export const CODEGRAPH_MARKER = `${BETABOT_MARKER}\n<!-- betabot:stage=codegraph -->`;
export const HYPOTHESES_MARKER = `${BETABOT_MARKER}\n<!-- betabot:stage=hypotheses -->`;
export const FIXES_MARKER = `${BETABOT_MARKER}\n<!-- betabot:stage=fixes -->`;
export const VERIFY_MARKER = `${BETABOT_MARKER}\n<!-- betabot:stage=verify -->`;
 ...
 */
export const REVIEW_MARKER = `${BETABOT_MARKER}\n<!-- betabot:review -->`;
...
export function buildCodegraphComment(input: BuildCodegraphCommentInput): string {
  ...
  lines.push("## Betabot · Stage 1: codegraph");
  lines.push("");
  lines.push(
  ...
  lines.push("---");
  lines.push(`_Run \`${input.runId}\` · betabot ${input.version} · stage 1 of the Betabot pipeline (codegraph only)._`);
  return lines.join("\n");
}
...
function hypothesisSection(repository: string, headSha: string, hypothesis: Hypothesis, index: number): string[] {
  ...
  lines.push(
    `<sub>Advisory \`${hypothesis.id}\`${hypothesis.priority ? ` · priority #${hypothesis.priority}` : ""} · ${hypothesis.ruleId} · ` +
      ...
      `dismiss with \`npm run betabot:learnings -- --dismiss ${hypothesis.id}\`</sub>`,
  );
  return lines;
}
...
export function buildHypothesesComment(input: BuildHypothesesCommentInput): string {
  ...
  lines.push("## Betabot · Stage 2: hypotheses");
  lines.push("");
  ...
  lines.push(`_${report.totals.hypotheses} advisory(ies) · stage 2 of the Betabot pipeline (hypotheses)._`);
  lines.push(`_Run \`${input.runId}\` · betabot ${input.version}._`);
  return lines.join("\n");
}
...
export function buildFixesComment(input: BuildFixesCommentInput): string {
  ...
  lines.push("## Betabot · Stage 3: fixes");
  lines.push("");
  ...
  lines.push(
    `_Total $${usage.totalCostUsd.toFixed(4)} of $${usage.maxCostUsd.toFixed(2)} budget · ` +
      `${report.totals.generated} draft patch(es) · stage 3 of the Betabot pipeline (fixes)._`,
  );
  lines.push(`_Run \`${input.runId}\` · betabot ${input.version}._`);
  return lines.join("\n");
}
...
export function buildVerifyComment(input: BuildVerifyCommentInput): string {
  ...
  lines.push("## Betabot · Stage 4: verify (autmpus loop)");
  lines.push("");
  lines.push(`\`${report.repository}\` · PR #${report.pullRequestNumber} · head \`${head}\``);
  lines.push("");
  lines.push(
  ...
  lines.push(
    `_Total $${usage.totalCostUsd.toFixed(4)} of $${usage.maxCostUsd.toFixed(2)} budget · ` +
      `stage 4 of the Betabot pipeline (verify)._`,
  );
  lines.push(`_Run \`${input.runId}\` · betabot ${input.version}._`);
  return lines.join("\n");
}
