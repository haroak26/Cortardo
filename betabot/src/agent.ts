 ...
 * betabot: the model thinks, calls read-only tools, sees the observations, and
 * finishes with a JSON answer. No sandbox, no writes, no shell.
 */
import type { BetabotMessage, BetabotModelClient } from "./model.ts";
...
export async function runAgent(input: AgentRunInput): Promise<AgentRunResult> {
  ...
  const history: BetabotMessage[] = [...(input.initialHistory ?? [])];
