...
export type BetabotStageName = "codegraph" | "hypotheses" | "fixes" | "verify";

/** Product pipeline: codegraph is internal and never publishes. */
export const BETABOT_STAGES: BetabotStageName[] = ["hypotheses", "fixes", "verify"];
...
export interface BetabotRunInput {
  repositoryId: string;
  pullRequestNumber: number;
  stages?: BetabotStageName[];
  runId?: string;
  dryRun?: boolean;
  maxFiles?: number;
}

export interface BetabotStageResult {
  stage: BetabotStageName;
  version: string;
  durationMs: number;
  headSha: string;
  summary: string;
...
export interface BetabotRunResult {
  runId: string;
  version: string;
  repository: string;
  pullRequestNumber: number;
  ...
  receipt: BetabotRunReceipt;
  stages: BetabotStageResult[];
}

export interface BetabotStageReceipt {
  stage: BetabotStageName;
  status: "ok";
  durationMs: number;
  summary: string;
...
export interface BetabotRunReceipt {
  runId: string;
  version: string;
  repository: string;
  pullRequestNumber: number;
  ...
  totalCalls: number;
  cacheHitRate: number;
  stages: BetabotStageReceipt[];
}

/** Internal handoff: stage 2 returns its structured report for stage 3. */
export interface HypothesisStageResult extends BetabotStageResult {
  report: HypothesisReport;
}

/** Internal handoff: stage 3 returns its structured report. */
export interface FixStageResult extends BetabotStageResult {
  report: FixReport;
...
export interface VerifyStageResult extends BetabotStageResult {
  report: VerifyReport;
