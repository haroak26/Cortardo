...
export async function runCodegenAgent(input: CodegenAgentInput): Promise<CodegenAgentResult> {
  ...
  let priorHistory: BetabotMessage[] = [];
  ...
  const collecting = new BetabotUsageCollectingClient(input.client);
    ...
    while (queue.length > 0) {
      ...
      try {
        const result = await runCodegenAgent({
          ...
          cacheKey: betabotCacheKey("codegen", input),
        });
...
export const DRAFT_SUGGESTION_BODY =
  "**Betabot draft fixes** — one-click suggestions generated from the hypotheses stage. " +
  "They are unverified drafts: review before applying.";

export async function replaceBetabotSuggestions(
...
export async function runFixesStage(input: FixStageInput): Promise<FixStageResult> {
  ...
  let body = buildFixesComment({ report: fixReport, runId: input.runId, version: BETABOT_VERSION });
  if (!input.dryRun && !input.deferSuggestions) {
    fixReport.suggestions = await replaceBetabotSuggestions({
      installationId: context.installationId,
      fullName: context.fullName,
      pullRequestNumber: input.pullRequestNumber,
      headSha: context.headSha,
    ...
    body = buildFixesComment({ report: fixReport, runId: input.runId, version: BETABOT_VERSION });
  }
