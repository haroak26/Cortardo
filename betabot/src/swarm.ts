...
import { BetabotUsageCollectingClient, type BetabotModelClient, type BetabotSwarmConfig } from "./model.ts";
import type {
...
export async function runSwarm(input: SwarmRunInput): Promise<SwarmAgentReport[]> {
  ...
  await runPool(tasks, input.config.concurrency, async ({ assignment, index }) => {
    ...
    try {
      ...
      const agentClient = new BetabotUsageCollectingClient(input.client);
