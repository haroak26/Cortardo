...
function deterministicUsage(configured: BetabotModelConfig, reason: string): ModelUsage {
  return {
...
export async function buildHypothesisReport(input: BuildHypothesisReportInput): Promise<HypothesisReport> {
  ...
  const coordinatorCacheKey = betabotCacheKey("coordinator", input);
  const swarmCacheKey = betabotCacheKey("swarm", input);

  if (input.modelClient === null) {
    usage = deterministicUsage(configured, "model disabled for this run (BETABOT_NO_MODEL)");
    ...
    const coordinator = new BudgetedModelClient(
      input.modelClient ?? createBetabotModelClient(configured),
      tracker,
      "coordinator",
    ...
    const swarm = swarmEnabled
      ? new BudgetedModelClient(
          input.swarmClient ?? createBetabotModelClient(swarmModelConfig(configured)),
          tracker,
          "swarm",
...
export async function runHypothesisStage(input: HypothesisStageInput): Promise<HypothesisStageResult> {
  ...
  const body = buildHypothesesComment({ report, runId: input.runId, version: BETABOT_VERSION });
