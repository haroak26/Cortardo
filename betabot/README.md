...
Betabot is the staged review agent that replaces the all-or-nothing CortardoBot pipeline.
...
[`../docs/betabot.md`](../docs/betabot.md).
...
The pipeline (`runBetabot`) runs stages 2-4 with publishing deferred, then publishes the
single review comment (`<!-- betabot -->` + `<!-- betabot:review -->`) and the verified
...
- **Cost**: minimal reasoning for codegen, a shared cacheable prompt prefix, `codegenTurns=3`
  ...
  `BETABOT_MAX_COST_USD`.

Only the priority fixes are drafted: hypotheses whose severity is in
`BETABOT_FIX_SEVERITIES` (default `critical,high`). Everything else is listed as skipped.
...
4. **Publish**: only verified edits are posted as native suggestions (each tagged
   `<!-- betabot:fix <id> -->` with a one-line context header), and the single review comment
...
- `src/index.ts` — `runBetabot()` pipeline, stage dispatch and shared loaders
...
```bash
npm run betabot:test               # unit tests
npm run betabot:e2e                # live stage 1 e2e against haroak26/Artificial-Gateway#6
npm run betabot:e2e:hypotheses     # live stage 2 e2e
npm run betabot:e2e:fixes          # live stage 2+3 e2e (publishes suggestions)
npm run betabot:e2e:verify         # live stage 2+3+4 e2e: cleans old bot comments, clones in E2B, verifies
npm run betabot:eval               # fixture recall/price against the planted-bug PRs
npm run betabot:eval -- --pr 7 --deterministic   # rules-only recall, no model calls
npm run betabot:learnings -- --repository owner/repo --list
npm run betabot:learnings -- --repository owner/repo --dismiss s_ab12cd34ef --reason "intentional"
```

The eval harness is for betabot itself: it runs stage 2 against `haroak26/Artificial-Gateway`
...
```bash
BETABOT_REPOSITORY=owner/repo BETABOT_PR=123 npm run betabot:e2e
BETABOT_REPOSITORY_ID=<uuid> BETABOT_DRY_RUN=1 npm run betabot:e2e:hypotheses
BETABOT_DRY_RUN=1 BETABOT_NO_MODEL=1 npm run betabot:e2e:hypotheses
```

`BETABOT_DRY_RUN=1` builds the report and comment without posting anything.
`BETABOT_NO_MODEL=1` skips the master-agent call and exercises the deterministic rules only.

Stage 2 model settings (base URL comes from `CORTADO_AI_BASE_URL`; the key resolves
`BETABOT_API_KEY` → `CORTADO_AI_API_KEY` → legacy names):

```bash
BETABOT_API_KEY=...                       # optional: swap an exhausted gateway key
BETABOT_MODEL=openai/gpt-5.6-terra        # coordinator: plan + synthesis (default)
BETABOT_SWARM_MODEL=openai/gpt-5.6-luna   # swarm investigators (default)
BETABOT_CODEGEN_MODEL=openai/gpt-5.6-sol  # codegen engineer (default)
BETABOT_SWARM_AGENTS=6                    # max assignments
BETABOT_SWARM_CONCURRENCY=3               # agents in flight at once
BETABOT_SWARM_TURNS=4                     # model turns per agent
BETABOT_SWARM_TOOLS=3                     # tool calls per turn
BETABOT_SWARM_SEARCH=0                    # disable gateway code search
BETABOT_LEARNINGS=0                       # ignore stored dismissals
BETABOT_MAX_FIXES=0                       # 0 = attempt every priority hypothesis
BETABOT_FIX_SEVERITIES=critical,high      # only these severities are fixed and verified
BETABOT_CODEGEN_CONCURRENCY=2             # codegen agents in flight
BETABOT_CODEGEN_TURNS=3                   # codegen turns per fix
BETABOT_MAX_COST_USD=0.50                 # shared soft cost ceiling (stages 2-4)
BETABOT_TARGET_COST_USD=0.40
BETABOT_STAGE2_BUDGET=0.22
BETABOT_STAGE4_RESERVE_USD=0.15           # only the verify stage may spend this
BETABOT_HYPOTHESES_MS=300000              # stage wall-clock budget
BETABOT_MODEL_TIMEOUT_MS=60000
BETABOT_REASONING=medium                  # coordinator/swarm reasoning
BETABOT_REASONING_CODEGEN=minimal         # codegen reasoning
BETABOT_VERIFY=1                          # 0 disables the sandbox stage
BETABOT_VERIFY_ATTEMPTS=4                 # 1 initial + repairs
BETABOT_VERIFY_MS=900000                  # stage 4 wall clock
BETABOT_VERIFY_COMMAND_TIMEOUT_MS=300000
BETABOT_VERIFY_COMMANDS=                  # newline-separated overrides (skips terra)
BETABOT_E2B_TEMPLATE=cortardo-review-v1
BETABOT_E2B_TIMEOUT_MS=900000
...
The legacy CortardoBot engine and its `server/lib/review/` integration have been removed;
engine 3.4 and earlier remain in git history (`cortardo-3.4-baseline` tag). Betabot is the
